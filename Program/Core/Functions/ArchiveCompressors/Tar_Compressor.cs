...
{
/// <summary> Initializes Compressing and Decompressing Functions for Files by using the Tar algorithm. </summary>

public class Tar_Compressor
{
/** <summary> Sets a Value which Contains Info about the Extension of a Tar file. </summary>
<returns> The Tar File Extension. </returns> */

private static readonly string tarFileExt = ".tar";
...
Path_Helper.AddExtension(ref outputPath, tarFileExt);

Archive_Manager.CheckDuplicatedPath(ref outputPath);
using FileStream outputFile = File.OpenWrite(outputPath);

using TarOutputStream compressionStream = new(outputFile);
TarEntry targetFileEntry = TarEntry.CreateTarEntry(inputFileName);

targetFileEntry.Size = inputFile.Length;

compressionStream.PutNextEntry(targetFileEntry);
...
/** <summary> Compresses the Contents of a Folder by using Tar Compression. </summary>
...
TarEntry targetFileEntry = TarEntry.CreateEntryFromFile(inputFilePath);

targetFileEntry.Name = relativePath;
compressionStream.PutNextEntry(targetFileEntry);
...
/** <summary> Compresses a FileSystem by using Tar Compression. </summary>
...
TarEntry targetFileEntry;

while( (targetFileEntry = decompressionStream.GetNextEntry() ) != null)
{
string relativePath = targetFileEntry.Name;
...
byte[] fileBuffers = new byte[Constants.ONE_KILOBYTE * 4];

int bytesRead;

while( (bytesRead = decompressionStream.Read(fileBuffers, 0, fileBuffers.Length)) > 0)
{
outputFile.Write(fileBuffers, 0, bytesRead);
}
...
/** <summary> Decompresses the Contents of a Folder by using Tar Compression. </summary>
...
Task_Manager.BatchTask(inputPath, outputPath, decompressAction, tarFileExt);
}
